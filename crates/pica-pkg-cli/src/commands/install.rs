...
use crate::{
  ...
  detect_luci_variant, detect_opkg_arches, detect_os, detect_platform, ensure_dir, ensure_dirs,
  install_via_feeds_or_ipk, make_temp_dir, manifest_get_first, normalize_uname,
  pkg_list_diff_added, pkgver_cmp_key, pkgver_ge, reorder_app_list, required_manifest_field,
  resolve_lang, run_hook, ver_ge, write_file_atomic, App, CliError, CliResult, Manifest, Selector,
  E_CONFIG_INVALID, E_INTEGRITY_INVALID, E_IO, E_MANIFEST_INVALID, E_PACKAGE_INVALID,
  E_PLATFORM_UNSUPPORTED, E_REPO_INVALID, E_RUNTIME, E_VERSION_INCOMPATIBLE, PICA_VERSION,
};
  ...
  if !Selector::is_structured(selector) && opkg_has_package(&parsed.appname) {
    let should_install_opkg = if app.options.non_interactive {
      matches!(
        ...
        crate::FeedPolicy::Ask | crate::FeedPolicy::FeedFirst | crate::FeedPolicy::FeedOnly
      )
    } else {
...
fn sha256_hex(content: &[u8]) -> CliResult<String> {
  ...
  Err(CliError::new(E_IO, "cannot compute sha256: need command 'sha256sum' or 'openssl'"))
}
