...
pub fn query_sync_info(app: &mut App, selector: &str) -> CliResult<()> {
  ...
  println!(
    "Size            : {}",
    best.size.map(|value| format_size_display(&value.to_string())).unwrap_or_default()
  );
...
pub fn query_info(app: &mut App, pkgname: &str) -> CliResult<()> {
  ...
  println!("AppName         : {}", manifest_get_first(manifest, "appname"));
  let program_url = {
  ...
  println!("LuCI URL        : {}", manifest_get_first(manifest, "luci_url"));
  ...
  println!("Size            : {}", format_size_display(&manifest_get_first(manifest, "size")));
  println!(
    "Installed At    : {}",
    entry
      .get("installed_at")
