...
fn file_sha256_hex(path: &Path) -> PicaResult<String> {
  ...
  if let Ok(output) = Command::new("openssl").arg("dgst").arg("-sha256").arg(path).output() {
    if output.status.success() {
      let text = String::from_utf8_lossy(&output.stdout);
  ...
  Err(PicaError::msg("cannot compute sha256: need command 'sha256sum' or 'openssl'"))
}
...
mod tests {
  ...
  fn unique_tmp_dir(name: &str) -> PathBuf {
    ...
    let dir =
      std::env::temp_dir().join(format!("pica-pack-test-{name}-{now}-{}", std::process::id()));
